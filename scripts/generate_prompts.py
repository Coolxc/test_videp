...
画风定位：iPad 手绘简笔画
- 像用 Apple Pencil 在 Procreate 上随手画的简笔画
- 纯黑色线条为主，线宽有自然的压感粗细变化（1-4px）
- 线条松散有机，有手绘的自然抖动，不死板不机械
- 每个元素用最少的笔画表达（5-15 笔），追求神韵不追求精确
...
SCENE_PROMPT_SYSTEM_PROMPT = """你是一位专业的 AI 图片生成 prompt 工程师，擅长为知识讲解视频生成 iPad 手绘简笔画风格的图片 prompt。
...
7. 各元素之间必须有清晰的空白分离（至少 15% 画布宽度），用于动画分区
