...
## 爆款内容分级策略 (Viral Strategy)

你必须根据文本内容的质量，将其分为以下三个级别并记录在 `groups.json` 的 `level` 字段中：

- **MASTERPIECE (Viral/爆款, 9分+)**:
    - **策略**: 生成 3 个 Hook 变体。视频场景必须严格遵循 **“一镜一物” (Rule of One)** 原则。
- **POTENTIAL (潜力, 7-8分)**:
    - **策略**: 生成 2 个 Hook 变体。
- **NORMAL (普通, 4-6分)**:
    - **策略**: 生成 1 个 Hook 变体。
...
### 1. 场景拆分：Rule of One (一镜一物)
为了消除“画面太碎”和“信息过载”，你必须强制执行以下规则：
- **每一镜 (Scene) 只能包含一个信息点/一个主体对象**。
- 如果一句话提到了三个事物（如“猫眼、门铃、门缝”），必须拆成三个 Scene。
- 只有当 Scene 切换时，画面才会清空重画，提高视觉确定感。

### 2. Hook 触发器 (Human Triggers)
生成的 Hook 文案必须利用以下三类人性钩子：
- **FEAR (恐惧/警示)**: 强调危险或不照做的严重后果。
    - *示例*: "⚠️ *千万别开门*！你房间的门铃已经动过手脚。"
- **CONTRARIAN (反常识)**: 打破思维定式，指出大众认知误区。
    - *示例*: "进酒店*第一件事客不是看床单*，而是摸一摸门铃。"
- **BENEFIT (获益/救命)**: 强调动作带来的关键避险价值。
    - *示例*: "这个动作不花一秒，但*关键时刻能保你一命*。"

### 3. Hook 格式规范
在 `hooks` 字典中生成的文案，使用 `*关键字*` 语法来指示黄色高亮显示。
...
**字段说明**：
- `score`: 脚本评估分（1-10）
- `level`: 内容级别 (normal / potential / viral)
- `hooks`: 包含三种类型的核心 Hook 文案
