...
def generate_storyboard(scenes, extra_info=None):
    """生成完整的 storyboard 数据"""
    if not scenes:
        return {
            'totalDuration': 0,
...
def main():
    ...
    try:
        ...
        # 提取 V2 新增字段
        extra_info = {}
        for key in ['score', 'level', 'hooks']:
            if key in groups_data:
                extra_info[key] = groups_data[key]
        
        storyboard = generate_storyboard(scenes, extra_info)
